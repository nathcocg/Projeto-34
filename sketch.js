...
function preload()
{
  ...
  emptyStar_img = loadAnimation("g_star1.png");
...
function setup() {
  ...
  star = createSprite(180,100);
  star.addAnimation("star",star_img);
  star.scale = 0.02;

  pointStar = createSprite(140,50);
  pointStar.addAnimation("empty",emptyStar_img);
  pointStar.addAnimation("star",star_img);
  pointStar.scale = 0.05;

  ellipseMode(RADIUS);

  createEdgeSprites(0,0);
}
...
function draw() 
{
  ...
  if(collide(fruit,star,40) == true) {
    pointStar.changeAnimation("star");
    star.destroy();
  }
